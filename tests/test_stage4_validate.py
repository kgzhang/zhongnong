"""Tests for stage4 validation engine (no models dependency)."""
...
# Lightweight test objects replacing models.Alternative etc.
class MockEntity:
...
def test_validate_required_fields_passes():
    entities = [MockEntity(entity_id="1", standard_name="thymol", alternative_class="Plant_Extract", evidence_text="...")]
    report = ValidationReport()
...
def test_validate_required_fields_fails():
    entities = [MockEntity(entity_id="", standard_name="", alternative_class="", evidence_text="")]
    report = ValidationReport()
    validate_required_fields(entities, "Alternative", ["entity_id", "standard_name", "alternative_class", "evidence_text"], report)
...
def test_validate_enum_values_ok():
    entities = [MockEntity(entity_id="1", experiment_id="E1", model_type="challenge")]
    report = ValidationReport()
...
def test_validate_enum_values_bad():
    entities = [MockEntity(entity_id="1", experiment_id="E1", model_type="challenged")]
    report = ValidationReport()
    validate_enum_values(entities, "Swine_Model", {"model_type": ["challenge", "normal"]}, report)
    assert len(report.fatals) >= 1
...
def test_validate_enum_values_skip_none():
    entities = [MockEntity(entity_id="1", experiment_id="E1", model_type="")]
    report = ValidationReport()
    validate_enum_values(entities, "Swine_Model", {"model_type": ["challenge", "normal"]}, report)
    assert len(report.fatals) == 0
...
def test_validate_foreign_keys():
    results = [
        MockEntity(entity_id="R1", doi="x", experiment_id="E1", matched_indicator="I1", matched_tissue="T1"),
        MockEntity(entity_id="R2", doi="x", experiment_id="E1", matched_indicator="I99", matched_tissue="T1"),
    ]
    indicators = {"I1": "ind_1"}
    tissues = {"T1": "tis_1"}
    report = ValidationReport()
